...
class ToggleServerMessages(name: String, description: String, private val f1DataService: F1DataService) : BotCommand(name, description){
    override fun execute(event: SlashCommandInteractionEvent) {
        if (event.member!!.hasPermission(Permission.ADMINISTRATOR)) {
            ...
            event.hook.sendMessage("Server Messages have been turned $toggleStatus").queue()
        }
        else {
